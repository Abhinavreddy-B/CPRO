...
int main()
{
    ...
    int pbility2[2][2] = {{0, 0}, {0, 0}};
    for (int i = 0; i < 127; i++)
    {
        array[i] = rand() % 2;
        if (i != 0)
        {
            pbility[array[i]][array[i - 1]]++;
            pbility2[array[i]][array[i - 1]]++;
        }
        // printf("%d",array[i]);
    }
    printf("\n");
    ...
    for (int i = 128; i < 100000; i++)
    {
        ...
        else
        {
            ...
            pbility2[array[pos + 1]][array[pos]]++;
            pos++;
        }
        // frequency2[rand()%2]++;
    }
    ...
    for (int j = 0; j < 2; j++)
    {
        printf("P(xi=%d/xi-1=%d) is %f\n", i, j, ((double)pbility2[i][j]) / (pbility2[0][j] + pbility2[1][j]));
    }
}